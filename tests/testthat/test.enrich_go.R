...
test_that("Check if enrich_go stops when species argument is invalid", {
  ...
  expect_equal(msg[6], "|-- INFO :  Enrichment analysis for cluster 2 ")
  expect_equal(msg[7], "|-- DEBUG :  Cluster 2")
  expect_equal(msg[8], "Number of gene names converted to EntrezId : 5")
  expect_equal(msg[9], "Number of gene names not converted to EntrezId : 0 ")

  expect_equal(msg[10], "|-- INFO :  Enrichment analysis for cluster 3 ")
  expect_equal(msg[11], "|-- DEBUG :  Cluster 3")
  expect_equal(msg[12], "Number of gene names converted to EntrezId : 3")
  expect_equal(msg[13], "Number of gene names not converted to EntrezId : 0 ")
})
...
test_that("Check if enrich_go results with all ontologies", {
  ...
  expect_equal(length(res_enrich@gene_cluster_annotations$`1`@geneSets), 1219)
  ...
  expect_equal(sum(res_enrich@gene_cluster_annotations$`2`@result$Count), 7)
  ...
  expect_equal(unname(res_enrich@gene_cluster_annotations$`3`@gene2Symbol), c(
    "CD200", "SIT1", "IGLL5"
  ))
