...
test_that("Checking output slots generated by gene_clustering() using keep_nn as FALSE", {
  ...
  expect_equal(round(Matrix::mean(res_no_keep_nn@data), 6), 0.639414)
  expect_equal(round(median(res_no_keep_nn@data), 6), 0)
  expect_equal(round(sd(res_no_keep_nn@data), 6), 1.329959)

  expect_equal(round(sum(Matrix::colMeans(res_no_keep_nn@data)), 1), 230.8)
  expect_equal(round(sum(Matrix::rowMeans(res_no_keep_nn@data)), 4), 194.382)
  ...
  expect_equal(res_no_keep_nn@parameters$fdr, 0.00005)
  ...
  expect_equal(res_no_keep_nn@parameters$k_graph, 5)
  expect_true(is.numeric(res_no_keep_nn@parameters$k_graph))
  expect_equal(length(res_no_keep_nn@parameters$k_graph), 1)
...
test_that("Checking output slots generated by gene_clustering() using keep_nn as TRUE", {
  ...
  expect_equal(round(Matrix::mean(res_keep_nn@data), 2), 0.64)
  expect_equal(round(median(res_keep_nn@data), 2), 0)
  expect_equal(round(sd(res_keep_nn@data), 1), 1.3)
  
  expect_equal(round(sum(Matrix::colMeans(res_keep_nn@data)), 1), 230.8)
  expect_equal(round(sum(Matrix::rowMeans(res_keep_nn@data)), 1), 194.4)
  ...
  expect_equal(res_keep_nn@parameters$fdr,  0.00005)
