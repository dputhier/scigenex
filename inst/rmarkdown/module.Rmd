
<!-- Some UPPERCASE (e.g FIG_PATH, and MODULE_NUMBER) variables are text to be replaced (see st_report() function). -->
<!-- NB: This code contains several global variables derived from gm_report() function arguments. -->



```{css echo=FALSE}
.panelset .panel-active {
    display: block !important;
}
```

```{r include = FALSE}
# from https://tinyurl.com/mwxj7544
DT::datatable(NULL)
```

```{r start_MOD_MODULE_NUMBER, include=FALSE, echo=FALSE, eval=TRUE}
suppressWarnings(rm(enrich_res))
suppressWarnings(rm(edo))
suppressWarnings(rm(enrich_res_test))
scigenex::set_verbosity(0)
library(knitr)
options(width=400)

knitr::opts_chunk$set(
  fig.path="FIG_PATH",
  fig.align = "center", 
  size = "tiny", 
  comment = "",
  fig.dpi=300,
  out.width="100%"
)
```


```{r, level_0_MOD_MODULE_NUMBER, eval=TRUE, echo=FALSE, results = "asis"} 
knitr::asis_output("# Module MODULE_NUMBER ")
``` 

```{r, level_0b_MOD_MODULE_NUMBER, eval=TRUE, echo=FALSE, results = "asis"} 
knitr::asis_output(paste0("Module name: ", names(cluster_set[MODULE_NUMBER, ]@gene_clusters), "\n\n"))
<<<<<<< HEAD
knitr::asis_output(paste0("Module size: ", length(cluster_set[MODULE_NUMBER, ]@gene_clusters[[1]])))
=======
knitr::asis_output(paste0("Module size: ", length(cluster_set@gene_clusters[[MODULE_NUMBER]])))
>>>>>>> 802939eb00a214623981e9019fc9d9afd89eb5ce
``` 

```{r, level_1_MOD_MODULE_NUMBER, eval=TRUE, echo=FALSE, results = "asis"} 
knitr::asis_output("## Expression {.panelset}")
``` 

```{r, heatmap_2_MOD_MODULE_NUMBER, eval="module_heatmap" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("This section provides various information about gene module MODULE_NUMBER.")
``` 

```{r, spatial_0_MOD_MODULE_NUMBER, eval="module_spatial" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Spatial expression ")
``` 

```{r, spatial_1_MOD_MODULE_NUMBER, eval="module_spatial" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("In this section we compute the average expression levels of the gene expression program (i.e cluster) using Seurat::AddModuleScore(). This metagene is mapped onto spatial coordinates.")
``` 

```{r, spatial_2_MOD_MODULE_NUMBER, eval="module_spatial" %in% section, echo=FALSE, results = "asis"} 

SpatialFeaturePlot_params$object <- seurat_object
SpatialFeaturePlot_params$features <- "MOD_MODULE_NUMBER"
base::do.call(Seurat::SpatialFeaturePlot, SpatialFeaturePlot_params)
``` 

```{r, heatmap_3_MOD_MODULE_NUMBER, eval="module_heatmap" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Static heatmaps ")
``` 

```{r, heatmap_4_MOD_MODULE_NUMBER, eval="module_heatmap" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("A heatmap with cells/spots as columns and genes as rows.") 
``` 

```{r heatmap_5_MOD_MODULE_NUMBER, echo=FALSE, eval="module_heatmap" %in% section }
plot_ggheatmap_params$object <- cluster_set[MODULE_NUMBER,]
plot_ggheatmap_params$ident <- Idents(seurat_object)
base::do.call(scigenex::plot_ggheatmap, plot_ggheatmap_params)
```
<br>


```{r, iheatmap_2_MOD_MODULE_NUMBER, eval="module_iheatmap" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Interactive heatmaps ")
``` 

```{r, iheatmap_3_MOD_MODULE_NUMBER, eval="module_iheatmap" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output(paste0("An interactive heatmap with cells/spots as columns and genes as rows. To limit the number of cells/spots displayed (which may  saturate the browser), the `subsample_by_ident()` function is used to randomly select a subset of cells/spots from each cluster. Here the maximum number of cells per population was : ", subsample_by_ident_params$nbcell, ".")) 
``` 

```{r iheatmap_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_iheatmap" %in% section }

subsample_by_ident_params$object <- cluster_set[MODULE_NUMBER,]
subsample_by_ident_params$ident <- Seurat::Idents(seurat_object)

plot_heatmap_params$object <- base::do.call(scigenex::subsample_by_ident, subsample_by_ident_params)
plot_heatmap_params$cell_clusters <- Idents(seurat_object)
base::do.call(scigenex::plot_heatmap, plot_heatmap_params)

```

```{r, iumap_2_MOD_MODULE_NUMBER, eval="module_umap" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### FeaturePlot")
``` 

```{r, iumap_3_MOD_MODULE_NUMBER, eval="module_umap" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("A UMAP in which mean gene module intensity is mapped to each cell/spot.") 
``` 

```{r iumap_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_umap" %in% section }
<<<<<<< HEAD
FeaturePlot_params_cp <- FeaturePlot_params
FeaturePlot_params_cp$object <- seurat_object
FeaturePlot_params_cp$features <- "MOD_MODULE_NUMBER"
FeaturePlot_params_cp$split.by <- NULL
base::do.call(Seurat::FeaturePlot, FeaturePlot_params_cp)
=======

FeaturePlot_used_params <- FeaturePlot_params
FeaturePlot_used_params$object <- seurat_object
FeaturePlot_used_params$features <- "MOD_MODULE_NUMBER"
FeaturePlot_used_params$split.by <- NULL
base::do.call(Seurat::FeaturePlot, FeaturePlot_used_params)
>>>>>>> 802939eb00a214623981e9019fc9d9afd89eb5ce
```



```{r, iumap_2_1_MOD_MODULE_NUMBER, eval="module_umap_by_ori" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### FeaturePlot (by origin)")
``` 

```{r, iumap_3_1_MOD_MODULE_NUMBER, eval="module_umap_by_ori" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("A UMAP in which mean gene module intensity is mapped to each cell/spot. The umap is splitted base on the origin of the cells/spots.") 
``` 

```{r iumap_4_1_MOD_MODULE_NUMBER, echo=FALSE, eval="module_umap_by_ori" %in% section }
<<<<<<< HEAD
FeaturePlot_params_cp <- FeaturePlot_params
FeaturePlot_params_cp$object <- seurat_object
FeaturePlot_params_cp$features <- "MOD_MODULE_NUMBER"
FeaturePlot_params_cp$combine <- FALSE
plot <- base::do.call(Seurat::FeaturePlot, FeaturePlot_params_cp)
patchwork::wrap_plots(lapply(plot, 
                             "+", 
                             ggplot2::theme(legend.position="none")))
=======

FeaturePlot_used_params <- FeaturePlot_params
FeaturePlot_used_params$object <- seurat_object
FeaturePlot_used_params$features <- "MOD_MODULE_NUMBER"
base::do.call(Seurat::FeaturePlot, FeaturePlot_used_params)
>>>>>>> 802939eb00a214623981e9019fc9d9afd89eb5ce
```

```{r, violin_2_MOD_MODULE_NUMBER, eval="module_violin" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Violon plot ")
``` 

```{r, violin_3_MOD_MODULE_NUMBER, eval="module_violin" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("Display the distribution module expression score across cell/spot populations. The score is computed using Seurat::AddModuleScore() function. The module score is the average expression of the genes in the module, normalized by the average expression of a set of control genes. The violin plot shows the distribution of this score across cell/spot populations.") 
``` 

```{r violin_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_violin" %in% section }
<<<<<<< HEAD
vlnPlot_params_cp <- vlnPlot_params
vlnPlot_params_cp$object <- seurat_object
vlnPlot_params_cp$features <- "MOD_MODULE_NUMBER"
vlnPlot_params_cp$split.by <- NULL
p <- base::do.call(Seurat::VlnPlot, vlnPlot_params_cp)
p + ggplot2::coord_flip()

=======

vlnPlot_used_params <- vlnPlot_params
vlnPlot_used_params$object <- seurat_object
vlnPlot_used_params$features <- "MOD_MODULE_NUMBER"
vlnPlot_used_params$split.by <- NULL
base::do.call(Seurat::VlnPlot, vlnPlot_used_params)
>>>>>>> 802939eb00a214623981e9019fc9d9afd89eb5ce
```

```{r, violin_by_ori_2_MOD_MODULE_NUMBER, eval="module_violin_by_ori" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Violon plot (by origin)")
``` 

```{r, violin_by_ori_3_MOD_MODULE_NUMBER, eval="module_violin_by_ori" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("Display the distribution module expression score across cell/spot populations. The score is computed using Seurat::AddModuleScore() function. The module score is the average expression of the genes in the module, normalized by the average expression of a set of control genes. The violin plot shows the distribution of this score across cell/spot populations. Here the diagram is splitted based on sample origins") 
``` 

```{r violin_by_ori_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_violin_by_ori" %in% section }
<<<<<<< HEAD
vlnPlot_params_cp <- vlnPlot_params
vlnPlot_params_cp$object <- seurat_object
vlnPlot_params_cp$features <- "MOD_MODULE_NUMBER"
vlnPlot_params_cp$cols <- color_split
vlnPlot_params_cp$pt.size <- 0
p <- base::do.call(Seurat::VlnPlot, vlnPlot_params_cp)
p + ggplot2::coord_flip()
```

```{r, dotplot_2_MOD_MODULE_NUMBER, eval="module_dotplot" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Dot plot")
``` 

```{r, dotplot_3_MOD_MODULE_NUMBER, eval="module_dotplot" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("Intuitive way of visualizing how feature expression changes across different cell/spot populations. The size of the dot encodes the percentage of cells/spots within a class, while the color encodes the AverageExpression level across all cells within a class.") 
``` 

```{r dotplot_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_dotplot" %in% section }
DotPlot_params_cp <- DotPlot_params
DotPlot_params_cp$object <- seurat_object
DotPlot_params_cp$features <- cluster_set@top_genes[[MODULE_NUMBER]]
DotPlot_params_cp$split.by <- NULL
p <- base::do.call(Seurat::DotPlot, DotPlot_params_cp)
p + ggplot2::theme(axis.text.x =  ggplot2::element_text(size=6, 
                                                        angle=45, vjust = 0.5), 
                   axis.text.y =  ggplot2::element_text(size=6)) 
=======
>>>>>>> 802939eb00a214623981e9019fc9d9afd89eb5ce

vlnPlot_used_params <- vlnPlot_params
vlnPlot_used_params$object <- seurat_object
vlnPlot_used_params$features <- "MOD_MODULE_NUMBER"
base::do.call(Seurat::VlnPlot, vlnPlot_used_params)
```

```{r, dotplot_by_ori_2_MOD_MODULE_NUMBER, eval="module_dotplot_by_ori" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Dot plot (by origin)")
``` 

```{r, dotplot_by_ori_3_MOD_MODULE_NUMBER, eval="module_dotplot_by_ori" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("Intuitive way of visualizing how feature expression changes across different cell/spot populations. The size of the dot encodes the percentage of cells/spots within a class, while the color encodes the AverageExpression level across all cells within a class") 
``` 

```{r dotplot_by_ori_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_dotplot_by_ori" %in% section }
DotPlot_params_cp <- DotPlot_params
DotPlot_params_cp$object <- seurat_object
DotPlot_params_cp$features <- cluster_set@top_genes[[MODULE_NUMBER]]
p <- base::do.call(Seurat::DotPlot, DotPlot_params_cp)
p + ggplot2::theme(axis.text.x =  ggplot2::element_text(size=6, 
                                                        angle=45, vjust = 0.5), 
                   axis.text.y =  ggplot2::element_text(size=6)) 
```


```{r, gene_info_1_MOD_MODULE_NUMBER, eval="module_genes" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Gene list ")
``` 

```{r, gene_info_2_MOD_MODULE_NUMBER, eval="module_genes" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("The genes related to that module.") 
``` 

```{r gene_info_3_MOD_MODULE_NUMBER, echo=FALSE, eval="module_genes" %in% section}
df <- data.frame(gene=names(scigenex::gene_cluster(cluster_set[MODULE_NUMBER,])))
```

```{r gene_info_4_MOD_MODULE_NUMBER, echo=FALSE, eval="module_genes" %in% section}
DT::datatable(
    df,
    extensions = 'Buttons', 
    options = list(
        dom = 'Bfrtip',
        buttons =
            list(list(
                extend = 'csv',
                title = "gene_list_MOD_MODULE_NUMBER"))))
```

```{r, level_2_MOD_MODULE_NUMBER, eval="module_cell_annot_IA" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("## Functional Annotation (IA-based) {.panelset}")
``` 

```{r, cell_annot_IA_1_MOD_MODULE_NUMBER, eval="module_cell_annot_IA" %in% section, echo=FALSE, results = "asis"} 
knitr::asis_output("### Gemini-based annotation")
``` 

```{r, cell_annot_IA_2_MOD_MODULE_NUMBER, eval="module_cell_annot_IA" %in% section, echo=FALSE, results = "asis"}
knitr::asis_output("What Gemini tells us about the likely cell identity or underlying function of this gene module:") 
``` 

```{r, cell_annot_IA_3_MOD_MODULE_NUMBER, echo=FALSE, eval="module_cell_annot_IA" %in% section }
prompt <- paste0("\n\nA biological sample from ", smp_region, " cell compartment from ", smp_organ, " at ",  smp_stage , " stage was analyzed using transcriptomic analysis.",
                  " A list of co-regulated genes that contains :",
                  paste0(sort(unlist(cluster_set[MODULE_NUMBER,]@gene_clusters)), collapse=", "),
                  " was found. What could be the cells, functions or pathways most likely related to theses genes ? The answers (n=4 in total) should be ordered by likelyhood and formatted as a list with the following item : 'infered cell type/function/pathway' (#1, #2, #3, #4) and 'rational'\n\n.", collapse=" ")

knitr::asis_output("#### Prompt")

knitr::asis_output(prompt)
cat("\n\n")
knitr::asis_output("#### IA Answer")
cat("\n\n")
suppressMessages(gemini.R::setAPI("APIKEY"))
answer <- NULL
try_err <- try(suppressMessages(gemini.R::gemini_chat(prompt)))

if(inherits(try_err, "try-error")){
   cat("\n\nSeems we have an issue with Gemini...\n\n") 
}else{
  if(!is.null(try_err) )
    knitr::asis_output(paste0("\n\n", try_err$outputs, "\n\n"))
}

cat("\n\n")
```


```{r, func_anno_MOD_MODULE_NUMBER, eval=TRUE, echo=FALSE, results = "asis"}

for(ann in annotation_src){
 
  cat(paste0("\n\n## Functional Enrichment (", ann ,") {.panelset}\n\n"))
  
  if("edo" %in% ls()){rm(edo)}
  
  gene_set <- setNames(unlist(cluster_set[MODULE_NUMBER,]@gene_clusters), NULL)
  
  # Some gene may have a version number at the end, e.g. "gene1~2" (e.g. in the case
  # of a clusterSet created from Seurat::FindAllMarkers() output).
  gene_set <- gsub(gene_set, pattern = "~[0-9]+$", replacement = "")
  try(suppressMessages(query_entrezid <- AnnotationDbi::select(get(bioc_org_db), 
                                                               keys = gene_set,
                                                               columns = c("ENTREZID", "SYMBOL"),
                                                               keytype = "SYMBOL")))
  
  try(enrich_res <- clusterProfiler::enrichGO(query_entrezid[,"ENTREZID"],
                                             OrgDb = bioc_org_db,
                                             ont = ann,
                                             readable = TRUE), silent = TRUE)
  
  if(!"enrich_res" %in% ls()){enrich_res <- data.frame()}
  
  try(edo <- enrichplot::pairwise_termsim(enrich_res, 
                                          method = "Resnik", 
                                          showCategory = 20, 
                                          semData = sem_sim[[ann]]), silent = TRUE)
  
  if(!"edo" %in% ls()){edo <- data.frame()}

  
  if("module_term_network" %in% section){
    cat("\n\n### Cnetplot (fdp)\n\n")
    cat("\n\nThe cnetplot() function from the clusterProfiler R package is used to visualize enriched functional categories (e.g., here GO BP) as a concept-gene network. It helps interpret the relationships between genes and the biological functions or pathways they are associated with. The cnetplot() function generates a bipartite network plot where nodes represent genes and terms (e.g., GO terms, pathways). Edges connect genes to the terms they are annotated with. The network helps illustrate how multiple terms share common genes, and how individual genes may be involved in multiple functions.\n\n") 
    
  cnetplot_params$x <- edo
  cnetplot_params$circular <- FALSE
  
  if(nrow(edo) > 0){
    
    try_err <- try(base::do.call(enrichplot:::cnetplot.enrichResult, cnetplot_params) + 
                     Seurat::NoLegend(), silent = TRUE)
    
    if(inherits(try_err, "try-error")){
       cat("\n\nError when creating diagram.\n\n") 
    }else{
      print(try_err)
    }
  }else{
    cat("\n\nNo diagram available.\n\n") 
  }
  
  }
  
    if("module_term_network_circ" %in% section){
    cat("\n\n### Cnetplot (circular)\n\n")
    cat("\n\nThe cnetplot() function from the clusterProfiler R package is used to visualize enriched functional categories (e.g., here GO BP) as a concept-gene network. It helps interpret the relationships between genes and the biological functions or pathways they are associated with. The cnetplot() function generates a bipartite network plot where nodes represent genes and terms (e.g., GO terms, pathways). Edges connect genes to the terms they are annotated with. The network helps illustrate how multiple terms share common genes, and how individual genes may be involved in multiple functions.\n\n") 
  
    
    cnetplot_params$x <- edo
    cnetplot_params$circular <- TRUE
    
    if(nrow(edo) > 0){
      
      try_err <- try(base::do.call(enrichplot:::cnetplot.enrichResult, cnetplot_params) + 
                       Seurat::NoLegend(), silent = TRUE)
      
      if(inherits(try_err, "try-error")){
         cat("\n\nError when creating diagram.\n\n") 
      }else{
        print(try_err)
      }
    }else{
      cat("\n\nNo diagram available.\n\n") 
    }
    
    }
  
  
    if("module_term_barplot_1" %in% section){
      
      cat("\n\n### Func. enr. barplot \n\n")
      cat("\n\nDotplot of enriched terms.\n\n") 
      
    if(!is.null(enrich_res)){
      if(nrow(enrich_res) > 0){
        try_err <- try(dplyr::mutate(enrich_res, 
                                   qscore = -log(p.adjust, base=10)) %>% 
                                   barplot(x="qscore", showCategory=15) + 
                        ggplot2::theme(axis.text.y = ggplot2::element_text(size=7)), silent = TRUE)
  
        
        if(inherits(try_err, "try-error")){
           cat("\n\nError when creating diagram.\n\n") 
        }else{
          print(try_err)
        }
      }else{
      cat("\n\nNo diagram available.\n\n") 
     }
    }else{
      cat("\n\nNo diagram available.\n\n") 
    }
  }

  if("module_term_barplot_2" %in% section){
    
    cat("\n\n### Func. enr. dotplot \n\n")
    cat("\n\nDotplot of enriched terms.\n\n") 
    
    if(!is.null(enrich_res)){
      if(nrow(enrich_res) > 0){
                try_err <- try(enrichplot::dotplot(enrich_res, showCategory=15) + 
                     ggplot2::theme(axis.text.y = ggplot2::element_text(size=7)), silent = TRUE)
        
        if(inherits(try_err, "try-error")){
           cat("\n\nError when creating diagram.\n\n") 
        }else{
          print(try_err)
        }
      }else{
      cat("\n\nNo diagram available.\n\n") 
     }
    }else{
      cat("\n\nNo diagram available.\n\n") 
    }
  }
  
  
  if("term_table" %in% section){
    cat("\n\n### Func. enr. table \n\n")
    cat("\n\nA table with various information related to enrichment.\n\n") 
   
    font.size <- "50%"
    if(nrow(edo) > 0){
      
      df_edo <- edo@result
      if(all(is.numeric(df_edo$RichFactor)))
        df_edo$RichFactor <- round(df_edo$RichFactor, 4)
      if(all(is.numeric(df_edo$FoldEnrichment)))
        df_edo$FoldEnrichment <- round(df_edo$FoldEnrichment, 4)
      if(all(is.numeric(df_edo$zScore)))
        df_edo$zScore <- round(df_edo$zScore, 4)
      if(all(is.numeric(df_edo$pvalue)))  
        df_edo$pvalue <- formatC(df_edo$pvalue, format = "e", digits = 2)
      if(all(is.numeric(df_edo$p.adjust)))  
        df_edo$p.adjust <- formatC(df_edo$p.adjust, format = "e", digits = 2)
      if(all(is.numeric(df_edo$qvalue)))   
      df_edo$qvalue <- formatC(df_edo$qvalue, format = "e", digits = 2)
      rownames(edo@result) <- NULL
    

      # from https://tinyurl.com/mwxj7544
      print(htmltools::tagList(DT::datatable(df_edo,
                                             options=list(
                                             initComplete = htmlwidgets::JS(
                                                        "function(settings, json) {",
                                                        paste0("$(this.api().table().container()).css({'font-size': '", 
                                                               font.size, "'});"),
                                                        "}"),
                                             dom = 'Bfrtip',
                                             buttons = list(list(extend = 'csv',
                                                             title = "gene_list_MOD_MODULE_NUMBER"))
                       ))))
    
    }else{
      cat("\n\nNo table available.\n\n") 
    }
        
  }
}

``` 

