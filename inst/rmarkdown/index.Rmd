...
```{r section_gen_info_17c, echo=FALSE, eval="exp_metrics" %in% section, echo=FALSE, results = "asis", warning=FALSE}
...
df <-  data.frame(seurat_object@meta.data$orig.ident, 
...
ggplot2::ggplot(df, mapping = ggplot2::aes(x=Cell_Identity, fill=Cell_Identity)) +
              ggplot2::geom_bar() +
...
colnames(df) <- c("Population", "Count")
...
df <-  as.data.frame(cbind(seurat_object@meta.data$orig.ident, as.character(Seurat::Idents(seurat_object))))
colnames(df) <- c("Origin", "Population")
df$Origin <- factor(df$Origin)
df$Population <- factor(df$Population, levels = rev(levels(Seurat::Idents(seurat_object))))
Population <- Origin <- Count <- NULL
ggplot2::ggplot(df, mapping = ggplot2::aes(x=Population, fill=Origin)) +
              ggplot2::geom_bar() +
              ggplot2::labs(x="Population", y="Count") +
              ggplot2::theme_minimal() +
              ggplot2::coord_flip() +
              ggplot2::theme(legend.position = "none") +
  ggplot2::scale_fill_manual(values = color_split)
...
df$Population <- factor(df$Population, levels = rev(levels(Seurat::Idents(seurat_object))))
Population <- Origin <- Count <- NULL
ggplot2::ggplot(df, mapping = ggplot2::aes(x=Population, fill=Origin, by = Population)) +
              ggplot2::geom_hline(yintercept = 0.5) +
              ggplot2::geom_bar(position="fill") +
              ggplot2::labs(x="Cell Identity", y="Fraction of Total") +
              ggplot2::theme_minimal() +
              ggplot2::coord_flip() +
              ggplot2::theme(legend.position = "none") +
...
Population <- Origin <- Count <- NULL
ggplot2::ggplot(df, mapping = ggplot2::aes(x=Population, fill=Origin)) +
              ggplot2::geom_bar(position="dodge", ggplot2::aes(y = ggplot2::after_stat(prop*100), group = Origin)) +
              ggplot2::labs(x="Population", y="Percent of Origin") +
              ggplot2::theme_minimal() +
...
df$Population <- factor(df$Population, levels = rev(levels(Seurat::Idents(seurat_object))))
Population <- Origin <- Count <- NULL
ggplot2::ggplot(df, mapping = ggplot2::aes(x=Origin, fill=Population, by = Origin)) +
              ggplot2::geom_bar(position="fill") +
              ggplot2::labs(x="Origin", y="Population") +
              ggplot2::theme_minimal() +
  ggplot2::geom_text(stat = ggstats::StatProp, position = ggplot2::position_fill(.5), size=2) 
...
df <-  data.frame(Origin=seurat_object@meta.data$orig.ident, Population=Seurat::Idents(seurat_object))
Population <- Origin <- Freq <- NULL
df <- dplyr::count(df, Origin, Population, name = "Freq")
df <- dplyr::mutate(df, "Percent Total" = round(100* Freq/ sum(Freq), 3))
df <- dplyr::group_by(df, Origin)
df <- dplyr::mutate(df, "Percent Origin" = round(100* Freq/ sum(Freq), 3))
DT::datatable(
    df,
    extensions = 'Buttons', 
    options = list(
...
plot_ggheatmap_params$object <- cluster_set
plot_ggheatmap_params$use_top_genes <- TRUE
plot_ggheatmap_params$hide_gene_name <- TRUE 
