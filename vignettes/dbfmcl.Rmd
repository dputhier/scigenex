...
# Transfert infered cell population to a DBFMCL object

The inferred populations can now be transferred into the DBFMCL object and represented using the plot_dbf function.

```{r}
dbf_seurat <- load_seurat(dbf, 
                          seurat_obj=pbmc_dbf,
                          dimplot_obj=dim_plot_dbf)
g_profiles <- plot_dbf(dbf_seurat,
       average_only = TRUE,
       floor=-1, 
       ceil=1)
g_profiles
