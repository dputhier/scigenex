...
This vignette demonstrates how to generate detailled gene module reports using the `gm_report()` function from the **scigenex** R package. The reports are using dynamic **bookdown** template and provide an integrated view of gene module analysis from any Seurat supported technology:
...
```{r loading2, echo=TRUE, eval=FALSE, warning=FALSE, results='hide', message=FALSE}
library(scigenex)
library(Seurat)
...
```{r chunk, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r chunk2, eval=TRUE, message=FALSE, warning=FALSE}
# Unset verbosity to avoid cluttering the output
set_verbosity(0)
...
```{r chunk3, eval=TRUE, message=FALSE, warning=FALSE}
# Create a temporary directory for the report output
tmp_dir <- tempdir()
...
```{r chunk4, eval=TRUE, message=FALSE, warning=FALSE}
# Create a temporary directory for the report output
tmp_dir <- tempdir()
