...
As shown using the `plot_cmp_genesets()` function, `Seurat::FindAllMarkers()` tends to find markers that are not so specific to cell populations. Thus this markers may be shared between gene modules. The size of the square is related to the Jaccard index.

```{r seurat_classes, echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE, results='hide'}
seurat_brain_mk <- Seurat::FindAllMarkers(brain1, only.pos = TRUE)
seurat_brain_mk <- seurat_brain_mk[seurat_brain_mk$p_val_adj <= 0.01, ]
